...
async function addBaddie(canvas, baddie) {
  const size = 65;
...
function addPlayer(canvas, playerData) {
  ...
  const textTemp = new fabric.Text(
    playerData.name.replace(/\w+/g, _.capitalize),
    {
      fontSize,
      top: size / 2 + 8,
      left: size,
  ...
  const group = new fabric.Group(
    [star, wrapCanvasText(textTemp, canvas, size + 30, size)],
    {
      id: playerData.id,
      borderColor: css.primary,
      cornerColor: css.primary,
...
function addVillain(canvas, villainData) {
  ...
  const textTemp = new fabric.Text(
    villainData.name.replace(/\w+/g, _.capitalize),
    {
      fontSize,
      top: size / 2 + 8,
      left: size,
...
function removeBaddieIfExists(canvas, id) {
